...
UPDATE sys_student SET class_name = 'Computer Science 2023-1' WHERE class_name IS NULL OR class_name = '';
UPDATE sys_student SET status = 'Enrolled' WHERE status IS NULL OR status = '';

-- 3. Clean selection table
TRUNCATE TABLE sys_selection;

-- 4. Create student-course selection relationships
...
-- 5. Ensure each selection has a corresponding grade record
...
-- 6. Fix GPA calculation in grades table
UPDATE sys_grade g
