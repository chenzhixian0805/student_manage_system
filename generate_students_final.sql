...
-- Generate 100 students using a simple self-join technique
-- This script generates students without complex loops or stored procedures

-- First, let's check how many students we already have
SELECT COUNT(*) AS current_students FROM sys_student;

-- Create a simple numbers table using a self-join trick
-- This will generate numbers from 1 to 100
WITH numbers AS (
...
-- Insert students into sys_student table
-- This will insert only the students we don't already have
INSERT INTO sys_student (user_id, student_id, name, gender, email, phone, class_name, status, college_id, major_id)
SELECT 
    16 + n AS user_id,
    CONCAT('2023', LPAD(n, 3, '0')) AS student_id,
    CONCAT('Student', LPAD(n, 3, '0')) AS name,
    'Male' AS gender,
    CONCAT('student', LPAD(n, 3, '0'), '@example.com') AS email,
    CONCAT('13800138', LPAD(n, 3, '0')) AS phone,
    'Computer Science 2023-1班' AS class_name,
    '在读' AS status,
    1 AS college_id,
    1 AS major_id
FROM numbers
WHERE NOT EXISTS (SELECT 1 FROM sys_student s WHERE s.student_id = CONCAT('2023', LPAD(n, 3, '0')));

-- Verify the final count
SELECT COUNT(*) AS final_students FROM sys_student;